...
static struct task_struct *find_task_by_host_pid(pid_t pid)
{
    struct pid *p;
...
static long get_rss_bytes(pid_t pid)
{
    ...
    rcu_read_lock();
    task = find_task_by_host_pid(pid);
    if (!task) {
        rcu_read_unlock();
        return -1;
    }
...
{
    printk(KERN_WARNING
           "[container_monitor] SOFT LIMIT container=%s pid=%d rss=%ldMB limit=%luMB\n",
           container_id, pid,
           rss_bytes >> 20, limit_bytes >> 20);
}
                         ...
                         unsigned long limit_bytes, long rss_bytes)
{
    ...
    printk(KERN_WARNING
           "[container_monitor] HARD LIMIT container=%s pid=%d rss=%ldMB limit=%luMB\n",
           container_id, pid,
           rss_bytes >> 20, limit_bytes >> 20);
}
...
static void timer_callback(struct timer_list *t)
{
    ...
    list_for_each_entry_safe(entry, tmp, &monitored_list, list) {
        ...
        printk(KERN_DEBUG "[container_monitor] container=%s pid=%d rss=%ldMB soft=%luMB hard=%luMB\n",
               entry->container_id, entry->pid,
               rss >> 20,
...
static long monitor_ioctl(struct file *f, unsigned int cmd, unsigned long arg)
{
    ...
    if (cmd == MONITOR_REGISTER) {
        printk(KERN_INFO
               "[container_monitor] registering container=%s pid=%d soft=%luMB hard=%luMB\n",
